...
#pragma mark -
@implementation LTLTableCell

@synthesize orderItem, deleteChecked, closeChecked, multiEditing, cellDelegate, LTLWeight;
...
- (void) setOrderItem:(OrderItem *)ordItem {
    ...
    LTLWeight = orderItem.item.itemLTLWeight;
    NSString *displayWeight = [LTLWeight stringValue];
    if (LTLWeight == [NSNumber numberWithInt:0])
    {
        displayWeight = @"N/A";
        lineCostLabel.text = displayWeight;
    }
    else
    {
        orderItem.item.itemLTLWeight = LTLWeight;
        lineCostLabel.text = [NSString stringWithFormat:@"%@ lbs", LTLWeight];
    }
...
- (void) updateDeleteButtonState {
	...
	if (self.cellDelegate != nil && [self.cellDelegate respondsToSelector:@selector(cartItemCell:markForDelete:)]) {
		[self.cellDelegate LTLTableCell:self markForDelete:self.deleteChecked];
	}
}
...
- (void) updateCloseButtonState {
	...
	if (self.cellDelegate != nil && [self.cellDelegate respondsToSelector:@selector(cartItemCell:markForClose:)]) {
		[self.cellDelegate LTLTableCell:self markForClose:self.closeChecked];
	}
}
