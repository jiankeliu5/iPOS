...
-(NSString *) getSignatureAsBase64 {
    ...
    UIImage *signatureImage = [self getSignatureAsImage];
    
    // Return a base 64 encoded string
    if ([self.signatureImageFormat isEqualToString:SIGNATURE_AS_PNG]) {
        imageData = UIImagePNGRepresentation(signatureImage);
    } else if ([self.signatureImageFormat isEqualToString:SIGNATURE_AS_JPG]) {
        imageData = UIImageJPEGRepresentation(signatureImage, 1.0f);
    }
...
#pragma mark -
#pragma mark OpenGL code - see GLPaint sample from Apple
...
- (void) renderLineFromPoint:(CGPoint)start toPoint:(CGPoint)end
{
	...
	count = MAX(ceilf(sqrtf((end.x - start.x) * (end.x - start.x) + (end.y - start.y) * (end.y - start.y)) / BRUSH_PIXELSTEP), 1);
	for(i = 0; i < count; ++i) {
